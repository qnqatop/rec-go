...
// Replica returns replica connection if exists or main connection.
func (db *DB) Replica() *pg.DB {
	db.replicaMutex.RLock()
	defer db.replicaMutex.RUnlock()
...
// SetReplica sets replica connection and starts watchdog.
func (db *DB) SetReplica(replica *pg.DB) {
	...
	go db.replicaWatchdog()
}

// HasReplica returns true if db has replica.
...
// replicaWatchdog checks replica connection.
func (db *DB) replicaWatchdog() {
	t := time.NewTicker(time.Second * 5)

	for range t.C {
		if _, err := db.replica.Exec(`SELECT 1`); err != nil {
			db.replicaMutex.Lock()
			db.replica = nil
			db.replicaMutex.Unlock()
...
// WithReplica creates new context with SQL Group "replica" for debug SQL logging.
func WithReplica(ctx context.Context) context.Context {
	return zm.NewSqlGroupContext(ctx, "replica")
}
