...
// WithSort is a function that adds user specific sort to query.
...
// WithColumnsExpr is a function that adds user specific columns to query using ColumnExpr.
func WithColumnsExpr(cols ...string) OpFunc {
	return func(query *orm.Query) {
		for _, col := range cols {
			query.ColumnExpr(col)
		}
	}
}
...
func WithJoinedIDs(ids []int, tableAlias, column string) OpFunc {
	return func(q *orm.Query) {
		...
		join := `INNER JOIN UNNEST(ARRAY[` + strings.Join(idsValues, ", ") + `]) WITH ORDINALITY AS ids("jID", "jOrderNumber") ON (?.? = "jID")`
